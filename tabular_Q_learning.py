    ...
    def q_learning(self, states, actions, rewards):
        ...
        for t in range(len(states)):
            ...
            if t < len(states) - 1:
                ...
                nextQValueAction0 = self.q_values[(nextState, 0)]
                nextQValueAction1 = self.q_values[(nextState, 1)]
                nextQValueAction2 = self.q_values[(nextState, 2)]

                QValueActionsForNextState = [nextQValueAction0, nextQValueAction1, nextQValueAction2]

                bestActionForNextState = max(enumerate(QValueActionsForNextState), key=lambda x: x[1])[0]

                nextQValue = self.q_values[(nextState, bestActionForNextState)]
            ...
            QValueActionsForState = [QAction0, QAction1, QAction2]
...
# plt.ylabel('rewards')
# plt.xlabel('episodes')
# plt.title("rewards for tabular q learnging, episodes = "+str(episodes) + ", epsilon decay = "+ str(qLearningAgent.epsilon) + ", steps = 1000")
